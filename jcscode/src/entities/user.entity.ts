import { Column, CreateDateColumn, Index, PrimaryGeneratedColumn, UpdateDateColumn, Entity } from 'typeorm';
...
export class User {
  ...
  @Column({ type: 'varchar', length: 50, nullable: true, unique: true }) username!: string | null;
  ...
  @Column({ type: 'varchar', length: 20, default: 'pending_verification' }) status!: string;
  ...
  @Column({ type: 'timestamptz', nullable: true }) phone_verified_at!: Date | null;
  @Column({ type: 'int', default: 0 }) failed_login_attempts!: number;
  @Column({ type: 'timestamptz', nullable: true }) locked_until!: Date | null;
  @Column({ type: 'timestamptz', nullable: true }) last_password_change!: Date | null;
  @Column({ type: 'boolean', default: false }) mfa_enabled!: boolean;
  @Column({ type: 'varchar', length: 255, nullable: true }) mfa_secret!: string | null;
  @Column({ type: 'text', array: true, nullable: true }) mfa_backup_codes!: string[] | null;
  @Column({ type: 'varchar', length: 5, default: 'pt-BR' }) language!: string;
  @Column({ type: 'varchar', length: 50, default: 'America/Sao_Paulo' }) timezone!: string;
